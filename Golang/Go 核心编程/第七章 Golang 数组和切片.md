...
(2) 传统的方法不够灵活，因此我们就引出需要学习的新的数据类型 =>数组。
```
...
3. 数组的各个元素的地址间隔是根据数组的类型决定。第二个元素的地址就是第一个元素的地址加上数组类型占用的字节大小。(int=>8个字节)
4. 案例演示
```
package main
...
func main() {
	...
	/*
		1.数组的地址可以通过数组名来获取&intArr
		2.数组的第一个元素的地址就是数组的首地址。
		3.第二个元素的地址就是第一个元素的地址加上数组类型占用的字节大小。(int=>8个字节)
	*/
...
3. 他们都是尽在 for 循环内部可见的局部变量。
4. 遍历数组元素的时候，如果不想使用下标 index ，可以直接把下标 index 标为下划线_。
5. index 和 value 的名称不是固定，即程序员可以自行指定，一般命令为 index 和 value 。
...
func main() {
	/*
		创建一个 byte 类型的26个元素的数组，分别放置‘A’-‘Z’。使用 for 循环访问所有元素并打印出来。提示：字符数据运算 ‘A’ + 1 -> 'B'。
	*/
...
func main() {
	...
	*/
	...
	for i := 0; i < len; i++ {
		intArr[i] = rand.Intn(100) //随机数大于0或小于100
	}
...
### 切片的基本介绍
1. 切片的英文是slice
2. 切片是数组的一个引用，因此切片是引用类型，在进行传递时，尊用引用传递的机制。
...
func main() {
	...
	/*
		1. slice 就是切片名
		2. intArr[1:3] 表示 slice 引用到 intArr 这个数组的起始下标为 1，最后的下标为3(但是不包含3)。
	*/
	slice := intArr[1:3]
...
5. 方式1和方式2的区别
```
(1) 方式1是直接引用数组，这个数组是事先存在的，陈新雇员是可见的。
(2) 方式2是通过 make 来创建切片，make 也会创建一个数组，是由切片在底层进行维护，程序员是看不见的。
```
...
```
说明：从 arr 数组下标为startIndex，取到下标为endIndex的元素(不含arr[endIndex])。
```
...
func main() {
	...
	slice := arr[:]
	slice2 := slice[1:2]
	slice2[0] = 100 //因为 arr，slice 和 slice2 指向的数据空间是同一个，因此slice[0]=100，其他的都改变。
...
(2) 按照上面的代码来看，a 和 slice 的数据空间是独立，相互不影响，也就是说 a[0] = 999, slice[1] 仍然是1。
```
