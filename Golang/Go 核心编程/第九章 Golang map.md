...
6. 代码说明
  - map 在使用前一定 make 。
  - map 的 key 不能重复，如果重复了，则以最后的 key-value 。
  - map 的 value 是可以相同的。
...
3. map 删除
```
delete(map, "key")，delete 是个内置函数，如果 key 存在，就删除该 key-value，如果 key 不存在，不操作，但是也不会报错。
```
4. 案例二
```
package main
...
8. 代码说明
```
(1) 如果 cites 这个 map 中存在 "no1" ，那么 findRes 就会返回 true，否则返回 false 。
```

## map 遍历
1. map 的遍历使用 for-range 的结构遍历
2. 案例一
```
package main
...
2. 案例一：使用一个 map 来记录 monster 的信息 name 和 age ，也就是说一个 monster 对应一个 map ，并且妖怪的个数可以动态的增加 => map 切片。
```
package main
...
func main() {
	...
	// 2. map 的 value 为结构体，包含学生的 名字，年龄，地址
