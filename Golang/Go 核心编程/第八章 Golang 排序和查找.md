...
2. 冒泡排序的规则
```
(1) 一共会经过 arr.length-1 轮数比较，每一轮将会确定一个数的位置。
...
```
3. 案例二：请对一个有序数组进行二分查找 {1, 8, 10, 89, 1000, 1234} ，输入一个数看看该数组是否存在此数，并且求出下表，如果没有就提示没有这个数。
```
package main
...
### 二维数组应用案例
1. 案例一：定义二维数组，用于保存三个班，每个班五名同学成绩，并求出每个班级平均分、以及所有班级平均分。
```
package main
