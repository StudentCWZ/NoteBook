...
3. 计算机语言发展史
  ...
  - 第二代计算机语言：低级语言。在低级语言当中已经引入了一些英文单词，例如变量赋值采用mv。低级语言主要以汇编语言为代表。
...
3. 编译阶段的过程
  ...
  - 编译结束后，可以将 class 文件拷贝到其他操作系统当中运行。【跨平台】
...
4. 运行阶段的过程
  ...
  - Classloader 会去硬盘上搜索A.class文件，找到该文件，则将该字节码文件装载到 JVM 当中。
...
10. Classloader 是如何搜索 HelloWorld.class 字节码文件。
  ...
  - 综上所述，如果 classpath 环境变量没有配置，每次执行 .class 程序的时候，需要在 Dos 命令窗口中先切换到.class字节码文件所在的路径下，然后运行。
...
public class HelloWorld{//定义一个公开的类，起名 HelloWorld
  ...
  */
  public static void main(String[] args){//定义一个公开的静态的主方法
    ...
    //输出中文【提醒：以下程序员两边双引号是“全角的”，这里的双引号不是 java 语法的一部分，这里全角的双引号只是一个普通的字符串】
